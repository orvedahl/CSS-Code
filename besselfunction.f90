...
!!$C
!!$C       ============================================================
!!$C       Purpose: This subroutine computes Bessel functions Jv(x) and
...
!!$C       ============================================================
!!$C
    Real*8 :: v, x, vm, V0, BJ(0:10), DJ(0:10), BY(0:10), DY(0:10)
    Integer :: NM, NS
...
15  FORMAT(1X,F6.2,4D16.8)
  ...
  SUBROUTINE JYV(V,X,VM,BJ,DJ,BY,DY) !,NEG)
!!$    C
!!$    C       =======================================================
...
!!$    C    
    ...
    Real*8, Intent(Out) :: VM
    Real*8, Intent(Out), Dimension(0:*) :: BJ, DJ, BY, DY
    !Integer, Intent(In) :: NEG
    Real*8 :: EL, EC, PI, RP2, X2, V0, VL, BJVL, R, VG, GA, GB, BJV0, BJV1, BJU0, BJU1, BYV0, BYV1
    Real*8 :: A, VV, PX, RP, QX, RQ, XK, A0, CK, SK, F, F0, F1, F2, CS, CS0, CS1, PV0, PV1, W0, R0, R1
    Integer :: N, K, L, K0, J, M
    ...
    IF (X.LE.12.0) THEN
       DO L=0,1
          ...
          R=1.0D0
          K = 1
          Do While ((K .le. 40) .and. (DABS(R).GT. DABS(BJVL)*1.0D-15))
    ...
    IF (X.LE.12.0D0) THEN
       IF (V0.NE.0.0) THEN
          DO L=0,1
             ...
             R=1.0D0
             IT = 1
             Do While ((IT .le. 40) .and. (DABS(R).GT.DABS(BJVL)*1.0D-15))
                R=-0.25D0*R*X2/(IT*(IT-VL))
                BJVL=BJVL+R
                IT = IT+1
             EndDo
       ...
       ELSE
          ...
          DO IT=1,30
             W1=W1+1.0D0/IT
             R1=-0.25D0*R1/(IT*(IT+1))*X2
             CS1=CS1+R1*(2.0D0*W1+1.0D0/(IT+1.0D0))
          EndDo
    ...
    BY(1)=BYV1
    DO IT=2,N
       BYVK=2.0D0*(V0+IT-1.0D0)/X*BYV1-BYV0
       BY(IT)=BYVK
       BYV0=BYV1
       BYV1=BYVK
    EndDo
    DY(0)=V0/X*BY(0)-BY(1)
    DO IT=1,N
       DY(IT)=-(IT+V0)/X*BY(IT)+BY(IT-1)
    EndDo
    VM=N+V0

!!$    !Reflection for negative order
!!$    IF (NEG .lt. 0) THEN 
!!$       DO K=0,N
!!$          SJ = BJ(K)
!!$          SY = BY(K)
...
!!$    C       ===================================================
!!$    C
    Real*8, Intent(In) :: X, MP
    Real*8 :: A0, F0, F1
    Integer :: N0, N1, NN, IT, MSTA1
    ...
    F1=ENVJ(N1,A0)-MP
    IT = 1
    Do While ((IT.le.20).and.(ABS(NN-N1).GT.1))
...
!!$    C
    Real*8, Intent(In) :: X, MP
    Integer, Intent(In) :: N
    Real*8 :: A0, F0, F1, HMP, EJN, OBJ
    Integer :: N0, N1, NN, IT, MSTA2
