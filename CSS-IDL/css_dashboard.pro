...
;css_dashboard.pro
;
;Kyle Augustson @ University of Colorado at Boulder Dec. 2012
;
; css_case keyword: select a css case
;----------------------------------------------------------------
...
pro handle_vort_event, ev
    ...
    for ir=0,state.nr-1 do begin
        ...
        for it=0,state.ntheta-1 do begin
            tmp = reform(ut[it,*,ir])
            b1 = reform(ut[it,state.nphi-5:state.nphi-1,ir])
            b2 = reform(ut[it,0:3,ir])
            tmp2 = compact_fd6(dpi,tmp,b1,b2,1,dtype=0)/(r[ir]*sint[it])
...
pro plot_shell_slices, nonuni=nonuni, root=root, recmax=recmax
    ...
    If (keyword_set(root)) Then Begin
        state.root_dir = root
    Endif Else Begin
        state.root_dir = '/freyr1/augustso/CSSDE/'
    EndElse
