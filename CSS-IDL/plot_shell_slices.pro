;----------------------------------------------------------------
;
;plot_shell_slices.pro
;
;Kyle Augustson @ University of Colorado at Boulder July 2009
;
;----------------------------------------------------------------
;Import some necessary procedures.
@read_shell_slice.pro
...
;    ctmaker_id = 0L
    ...
    state = {drawing_widget:drawing_widget,quantity_widget:quantity_widget,window_size:window_size, radius_widget:radius_widget, $
             ...
             num_records:num_records,current_quantity_selection:current_quantity_selection,nonuni:nonuni, $ 
             max_slider:max_slider, min_slider:min_slider,rec_slider:rec_slider, $
             current_record_selection:current_record_selection, current_radius_selection:current_radius_selection, root_dir:root_dir, recmax:recmax}
end
...
;            'ctmaker' : handle_ctmaker_event, ev
            ...
            'enthalpy': handle_enthalpy_event, ev
            else: ;do nothing
       endcase
    endelse
end

pro handle_enthalpy_event, ev
    common shell_slice_block, data, r, phi, theta, file, quantity_names, min_value, max_value, quantities, wr, fen
    if(n_elements(data) le 1) then return
    WIDGET_CONTROL, ev.top, GET_UVALUE=state
    state.current_quantity_selection=17
    WIDGET_CONTROL, ev.top, SET_UVALUE=state
    fen = dblarr(state.ntheta,state.nphi,state.nr)
    rho = reform(data(*,*,*,0,state.current_record_selection))
    t = reform(data(*,*,*,1,state.current_record_selection))
    ur = reform(data(*,*,*,4,state.current_record_selection))
    cp = 3.4d8
    lsun = 3.86d33
    for ir=0,state.nr-1 do begin
        ruavg = total(rho(*,*,ir)*ur(*,*,ir))/double(state.nphi)/double(state.ntheta)
        tavg = total(t(*,*,ir))/double(state.nphi)/double(state.ntheta)
        fen(*,*,ir) = 4d0*!dpi*r(ir)^2*(rho(*,*,ir)*ur(*,*,ir)-ruavg)*cp*(t(*,*,ir)-tavg)/lsun
        print, 'Mean F_en at r(ir)=', r(ir)/6.96d10, ' is ', mean(fen(*,*,ir))
    endfor
    max_value = max(fen(*,*,state.current_radius_selection))
    min_value = min(fen(*,*,state.current_radius_selection))
...
pro handle_vort_event, ev
    ...
    for ir=0,state.nr-1 do begin
        for ip=0,state.nphi-1 do begin
            tmp = sint*reform(up[*,ip,ir])
            ; ibc should be 1 for Dirichlet
            tmp2 = compact_fd6(dti,tmp,0d0,0d0,1,dtype=0)/r[ir]/sint
            wr[*,ip,ir] = tmp2
        endfor
        for it=0,state.ntheta-1 do begin
            tmp = reform(ut[it,*,ir])
            b1 = reform(ut[it,state.nphi-4:state.nphi-1,ir])
            b2 = reform(ut[it,0:3,ir])
            ; ibc should be 3 for periodic
            tmp2 = compact_fd6(dpi,tmp,b1,b2,3,dtype=0)/(r[ir]*sint[it])
...
pro handle_radius_change, ev
    ...
    Case iq of
        ...
        17: begin
            max_value = max(fen(*,*,ir))
            min_value = min(fen(*,*,ir))
        end
        else: begin
            max_value = max(data(*,*,ir,iq,state.current_record_selection))
            min_value = min(data(*,*,ir,iq,state.current_record_selection))
        end
    EndCase
    WIDGET_CONTROL, state.min_slider, SET_VALUE=[min_value,min_value,max_value]
    WIDGET_CONTROL, state.max_slider, SET_VALUE=[max_value,min_value,max_value]
...
pro handle_drawing_event,ev, ir, iq, irec, window_id
    ...
    Case iq of
        16: begin
            draw = Transpose(Reform(wr(*,*,ir)))
        end
        17: begin
            draw = Transpose(Reform(fen(*,*,ir)))
        end
        else: begin
    ...
    If (mnv ne mxv) Then Begin
        display_shell_slice, lat, lon, draw, /publication, /grid, minv=mnv, maxv=mxv, res_scale=0.3, $
          orthographic=[0.5d0*(min(lon)+max(lon)),0.5d0*(min(lat)+max(lat))], latlon_window=limits, /nolines
    Endif
    ;stop
end
...
;    WIDGET_CONTROL, state.ctmaker_id, GET_VALUE=ct_value
    ...
    ;Create a button widget to compute enthalpy flux.
    fen_button = WIDGET_BUTTON(options_column,VALUE='Compute F_en',UVALUE='enthalpy')
...
;    ctmaker_widget = ctmaker(BASE_WIDGET=options_column,UVALUE='ctmaker')
    rec_slider = WIDGET_SLIDER(options_column,MAXIMUM=state.num_records,MINIMUM=0,VALUE=state.current_record_selection,UVALUE='rec_slider')
    ;Min, max sliders
...
pro handle_file_load_event, ev
    ...
    file = DIALOG_PICKFILE(PATH=state.root_dir,TITLE='Select Shell Slice File to Read')
    ...
    read_shell_slice, file, phi=phi, theta = theta, radii = r, data = data, QUANTITIES = quantities, nonuni=state.nonuni, recmax=state.recmax
    print, 'file loaded'
    print, 'size of data', size(data)
    ;phi = 180.0d0*phi/!PI
    ;theta = 180.0d0*theta/!PI-90.0d0    

    If (state.recmax eq -1) Then Begin
       num_records = n_elements(data(0,0,0,0,*))
       state.num_records = num_records
    Endif Else Begin
       num_records = state.recmax
       state.num_records = state.recmax
    EndElse
    ...
    If (num_records eq 1) Then Begin
        state.num_records = 2
        tmp = dblarr(state.ntheta,state.nphi,state.nr,state.nq,2)
        tmp[*,*,*,*,0]=data
        tmp[*,*,*,*,1]=data
        data = tmp
        tmp = 0
    ...
    WIDGET_CONTROL, state.quantity_widget, SET_COMBOBOX_SELECT=4

    ;Create a string array of radii names.
    ;Add the current name list to the combobox.
    WIDGET_CONTROL, state.radius_widget, GET_VALUE=temp
    for i=0,N_R-1 do begin
        WIDGET_CONTROL,state.radius_widget,COMBOBOX_ADDITEM=string(r(i)/6.96d10)
        WIDGET_CONTROL, state.radius_widget, SET_COMBOBOX_SELECT=i+1
    endfor
...
pro plot_shell_slices, nonuni=nonuni, root=root, recmax=recmax
    common shell_slice_block, data, r, phi, theta, file, quantity_names, min_value, max_value, quantities, wr, fen
    data=0
    r=0
    theta=0
    ...
    If (keyword_set(nonuni)) Then state.nonuni = nonuni
    If (keyword_set(recmax)) Then state.recmax = recmax
