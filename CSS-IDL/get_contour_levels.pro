...
function get_contour_levels,field,nlev,spacing=spacing,mn=mn,mx=mx,pct=pct, $
...
if n_params() ne 2 then begin
  print,'  levels = get_contour_levels(field,nlev,spacing=spacing,'+$
    'mn=mn,mx=mx,pct=pct)'
  return,-1
endif
...
sfield=sort(field)
mn=field(sfield(long(pct*n_elements(field))))  ;  histogram trim (bottom)
mx=field(sfield(long((1-pct)*n_elements(field))))  ;  histogram trim (top)
meanf=mean(field)
