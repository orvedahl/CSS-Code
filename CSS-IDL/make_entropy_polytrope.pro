...
pro make_entropy_polytrope, filename, poly_idx, new_nr, smod=smod, dencon=dencon, reverse=reverse, dsmin=dsmin, rmin=rmin, rmax=rmax, simple=simple, Ra=Ra
   ...
   if (not keyword_set(dsmin)) then begin
    ...
    EndIf Else Begin
        ...
        imid = max(where(rad le sqrt(rmin*rmax)))
        
        muexp = poly_idx
        kapexp = 0d0
        k0 = delta*rho1*T1*rmin*(rmax-rmin)*sqrt(Cp*T1*gam1*(poly_idx+1d0)*(1d0-(poly_idx+1d0)*gam1/gamma)/Pr/Ra1/gamma*theta(imid)^(2d0*poly_idx-muexp-kapexp))/rmax
        mu = Pr*k0*theta^(muexp)/T1
        kr = k0*theta^(kapexp)
        kap = kr/poly_rho/poly_T
        ...
        Ck = k0/(rho1*T1*(rmax-rmin)*sqrt(Cp*T1*gam1/gamma))
        poly_Ra = rho1^2*T1^3*Cp*rmax^2*rmin^2*delta^2*gam1*(poly_idx+1d0)*((poly_idx+1d0)/gamma-poly_idx)*(rmax-rmin)^2*theta^(2*poly_idx-kapexp-muexp)/rad^4/gamma/Pr/k0^2
        grav = GM/rad^2
